...
import {
  PROVIDERS, PROVIDER_ORDER, testConnection, listOllamaModels, LlmError,
  type OllamaModel,
...
/**
 * Model picker for Ollama specifically.
 *
 * Every other provider has a short, hand-picked model list because there are
 * only ever a few models worth offering. Ollama is the opposite: the only
 * models worth offering are whatever the person has actually pulled, and that
 * list lives on their machine, not in this codebase. So it's fetched from the
 * server itself, with a manual fallback for when the fetch fails (server not
 * running yet, CORS not opened) so a name can still be typed in by hand.
 */
function OllamaModelField({
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setModels(list)
      // Nothing picked yet, or the pick was since removed — default to the
      // first real model rather than leaving the select on an empty value.
      if (list.length && !list.some((m) => m.name === model)) onChange(list[0].name)
    } catch (e) {
      setModels(null)
      setErr(e instanceof LlmError ? e.message : 'Model listesi alınamadı.')
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    void refresh()
    // Re-fetch when the address changes — a different port is a different
    // server, with a different set of pulled models.
    ...
    <Field label="Model">
      <div className="flex gap-1.5">
        {models && models.length > 0 ? (
          <select className="field field-sm" value={model} onChange={(e) => onChange(e.target.value)}>
            {models.map((m) => (
        ...
        <button
          ...
          aria-label="Model listesini yenile"
        >
          <Icons.refresh className={`w-3.5 h-3.5${loading ? ' spin' : ''}`} />
        </button>
      </div>
      {err && (
        <p className="text-[0.66rem] mt-1 leading-snug" style={{ color: 'var(--rose)' }}>
          {err}
        </p>
      )}
      {models && models.length === 0 && !err && (
        <p className="text-[0.66rem] mt-1" style={{ color: 'var(--ink-mute)' }}>
          Hiç model bulunamadı. Terminalden <code>ollama pull llama3.1</code> gibi bir komutla bir model indir.
        </p>
      )}
    </Field>
  )
...
function LlmSettingsBlock() {
  ...
  const baseUrl = llm.baseUrls[llm.provider] ?? ''
  ...
  const runTest = async () => {
    ...
    try {
      const text = await testConnection({
        ...
        apiKey: key,
        model,
        baseUrl: def.configurableBaseUrl ? baseUrl : undefined,
      })
        ...
        <>
          <div className="flex gap-1 p-1 rounded-full" style={{ background: 'var(--surface-2)' }}>
            {PROVIDER_ORDER.map((id) => (
              ...
              >
                {PROVIDERS[id].label}
                {(llm.keys[id] || (PROVIDERS[id].noKeyRequired && llm.models[id])) ? ' ✓' : ''}
              </button>
            ))}
          </div>
          ...
          {llm.provider === 'ollama' ? (
            <OllamaModelField
              ...
              model={model}
              onChange={(m) => patch({ models: { ...llm.models, ollama: m } })}
            />
          ) : def.models.length > 0 ? (
            <Field label="Model">
              <select
          ...
          {def.configurableBaseUrl && (
            <Field label="Adres" hint="/v1/chat/completions bu adrese eklenir">
              <input
                ...
                value={baseUrl}
                onChange={(e) => patch({ baseUrls: { ...llm.baseUrls, [llm.provider]: e.target.value } })}
              />
            </Field>
          )}

          <div className="flex items-center gap-2">
            <button className="btn btn-xs" disabled={testing || (!key.trim() && !def.noKeyRequired)} onClick={runTest}>
          ...
          <p className="text-[0.66rem] leading-snug" style={{ color: 'var(--ink-mute)' }}>
            {def.noKeyRequired ? (
              <>
                Her şey kendi bilgisayarında kalır — sunucuya hiçbir şey gönderilmez. {def.hint}
              </>
            ) : (
              <>
                Anahtar yalnızca bu tarayıcıda saklanır ve doğrudan sağlayıcıya gider — arada sunucumuz yok. Paylaşılan
