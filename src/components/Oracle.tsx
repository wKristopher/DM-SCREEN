...
export function CommandBar() {
  ...
  /**
   * A real question ("Bu tapınağın rahibi kimden korkuyor?") wants a real
   * answer, not a d20 fate roll — but the fate roll is still exactly right
   * for a yes/no check mid-combat ("Tuzak var mı?"), and it is instant where
   * an AI call is not. So: ask the connected provider when one is configured,
   * and fall back to the local oracle — silently, so the bar never just sits
   * there — if it isn't, or if the call fails.
   */
  const askOracle = useCallback(
    (question: string) => {
      const def = PROVIDERS[llm.provider]
      const key = llm.keys[llm.provider] ?? ''
      const model = (llm.models[llm.provider] ?? def.defaultModel).trim()
      const ready = llm.enabled && !!model && (!!key.trim() || !!def.noKeyRequired)

      const fallback = (why?: string) => {
        if (why) setToast(why)
        pushResult(G.askOracle(question))
        setTab('oracle')
      }
      ...
      setToast('Kâhine soruluyor…')
      void runLlm(
        {
          provider: llm.provider,
          apiKey: key,
          model,
          baseUrl: def.configurableBaseUrl ? llm.baseUrls[llm.provider] : undefined,
        },
        'ask',
        question,
      )
        .then((text) => {
          if (!text.trim()) {
            fallback('Kâhin boş yanıt verdi — yerel kâhine düşülüyor.')
            return
          }
          pushResult({
            kind: 'oracle',
            accent: 'violet',
            title: question,
            subtitle: `Kâhin · ${def.label} · ${model}`,
            fields: [],
            prose: text,
          })
          setTab('oracle')
        ...
        .catch((e) => {
          fallback(e instanceof LlmError ? e.message : 'Kâhine ulaşılamadı — yerel kâhine düşülüyor.')
        })
    },
    [llm, pushResult, setTab, setToast],
  )
  ...
  const submit = useCallback(
    ...
    [extra, standalone, pushResult, pushRoll, setLookup, setTab, setToast, logRoll, askOracle],
  )
...
function LlmBox() {
  ...
  if (!llm.enabled || (!key && !def.noKeyRequired)) {
    return (
      <div className="rounded-2xl p-3.5 text-[0.78rem]" style={{ background: 'var(--bg-deep)', color: 'var(--ink-mute)' }}>
        <p className="mb-2">
          ...
          İstersen Claude, ChatGPT, Gemini ya da kendi bilgisayarında çalışan bir Ollama modeli bağlayıp serbest metin
          (betimleme, NPC replikleri, seans özeti) de aldırabilirsin.
        </p>
