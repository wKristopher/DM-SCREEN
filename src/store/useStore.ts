...
export interface LlmSettings {
  ...
  /** Only meaningful for providers with configurableBaseUrl — kept per
   * provider so a custom OpenAI-compatible address and a local Ollama
   * address don't overwrite each other when switching between them. */
  ...
  persist(
    (set, get) => ({
      ...
      restore: (data) =>
        set((s) => {
          ...
          const incomingLlm = (incoming.llm ?? {}) as Partial<LlmSettings> & { baseUrl?: string }
          const legacyBaseUrl = incomingLlm.baseUrl
          const baseUrls =
            legacyBaseUrl && !incomingLlm.baseUrls?.compatible
              ? { ...incomingLlm.baseUrls, compatible: legacyBaseUrl }
              : incomingLlm.baseUrls
          return {
            ...
            settings: {
              ...
              llm: {
                ...
                ...incomingLlm,
                baseUrls: baseUrls ?? {},
                keys: s.settings.llm.keys,
                enabled: s.settings.llm.enabled,
              },
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        // Pre-per-provider shape carried a single shared `baseUrl` string,
        // which only ever meant the OpenAI-compatible provider (the only one
        // configurable back then) — fold it in rather than losing it.
        const legacyBaseUrl = (saved.llm as unknown as { baseUrl?: string } | undefined)?.baseUrl
        if (legacyBaseUrl && !llm.baseUrls.compatible) {
          llm.baseUrls = { ...llm.baseUrls, compatible: legacyBaseUrl }
        }
